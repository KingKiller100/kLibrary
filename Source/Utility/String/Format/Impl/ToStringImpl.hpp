...
namespace klib::kString::secret::impl
{
	...
	constexpr void ToStringImpl(
		std::basic_string<Char_t>& outFmt
		, const size_t argIndex
		, const size_t placeholderPos
		, type_trait::SizeCondConstRef_t<T> arg
		, type_trait::SizeCondConstRef_t<Ts> ...argPack
	)
	{
		using namespace stringify;
		using Str_t = std::basic_string<Char_t>;
		...
		try
		{
			size_t openerPos = FindOpenerPosition( outFmt, argIndex, placeholderPos );
