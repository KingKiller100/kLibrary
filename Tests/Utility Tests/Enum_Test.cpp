...
namespace kTest::utility
{
	...
	public:
		enum InternalEnum_t : underlying_t
		{
	...
	private:
		struct secret_impl_TestResults
		{
			static constexpr auto size = IDENTITY(
				COUNT(A_STAR = (static_cast<size_t>(1) << 0) , A = (static_cast<size_t>(1) << 1) , B = (static_cast<
						size_t>(1) << 2) , C = (static_cast<size_t>(1) << 3) , D = (static_cast<size_t>(1) << 4) , E = (
						static_cast<size_t>(1) << 5) , F = (static_cast<size_t>(1) << 6) , PASS = A_STAR | A | B | C ,
					FAIL
					= D | E | F) );
			static constexpr std::array<underlying_t, size> values = {
				IDENTITY(
					IGNORE_ASSIGN(A_STAR = (static_cast<size_t>(1) << 0) , A = (static_cast<size_t>(1) << 1) , B = (
							static_cast<size_t>(1) << 2) , C = (static_cast<size_t>(1) << 3) , D = (static_cast<size_t>(
								1)
							<< 4) , E = (static_cast<size_t>(1) << 5) , F = (static_cast<size_t>(1) << 6) , PASS =
						A_STAR |
						A | B | C , FAIL = D | E | F) )
			};
			static constexpr std::array<std::string_view, size> raw_names = {
				IDENTITY(
					STRINGIZE(A_STAR = (static_cast<size_t>(1) << 0) , A = (static_cast<size_t>(1) << 1) , B = (
							static_cast<size_t>(1) << 2) , C = (static_cast<size_t>(1) << 3) , D = (static_cast<size_t>(
								1)
							<< 4) , E = (static_cast<size_t>(1) << 5) , F = (static_cast<size_t>(1) << 6) , PASS =
						A_STAR |
	...
	public:
		constexpr TestResults( const InternalEnum_t value ) : value( value )
		{
		}

		constexpr TestResults& operator=( const InternalEnum_t value )
		{
			this->value = value;
			return *this;
		}

		constexpr operator InternalEnum_t() const { return static_cast<InternalEnum_t>(value); }
		USE_RESULT constexpr InternalEnum_t ToEnum() const { return static_cast<InternalEnum_t>(value); }
		USE_RESULT constexpr underlying_t ToUnderlying() const { return value; }
		USE_RESULT constexpr bool MaskCmp( InternalEnum_t target ) const { return MaskCmp( target, true, false ); }

		template <typename T1, typename T2, typename = std::enable_if_t<std::is_convertible_v<T2, T1>>>
		USE_RESULT constexpr std::decay_t<T1> MaskCmp( InternalEnum_t mask, T1&& successState, T2&& failState ) const
		{
			if ( mask & value ) return successState;
			return failState;
		}

		USE_RESULT constexpr bool Compare( InternalEnum_t target ) const { return Compare( target, true, false ); }

		template <typename T1, typename T2>
		USE_RESULT constexpr std::decay_t<T1> Compare( InternalEnum_t target, T1&& successState, T2&& failState ) const
		{
		...
		USE_RESULT constexpr bool operator<( const TestResults& other ) const { return value < other.value; }
		USE_RESULT constexpr bool operator<=( const TestResults& other ) const { return value <= other.value; }

		template <typename T>
		USE_RESULT constexpr bool operator<( const T& other ) const { return value < other; }
	...
	private:
		...
		static constexpr InternalEnum_t FromStringImpl( const char* s, size_t index )
		{
			...
			const auto ret = matches
				                 ? static_cast<InternalEnum_t>(secret_impl_TestResults::values[index])
	...
	bool EnumTester::CompareTest()
	{
		...
		{
			constexpr TestResults tr = TestResults::PASS;
			constexpr auto pass = static_cast<TestResults::InternalEnum_t>(
				TestResults::A_STAR | TestResults::A | TestResults::B | TestResults::C);
			VERIFY_COMPILE_TIME( tr.Compare(static_cast<TestResults::InternalEnum_t>(pass)) == true );
		}
