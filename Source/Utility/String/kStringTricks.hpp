...
namespace klib
{
	namespace kString
	{
		...
		template <class CharT = char, class = std::enable_if_t<
			          type_trait::Is_Char_V<CharT>
		          >>
		USE_RESULT constexpr StringWriter<CharT> Replace(
			const CharT* str, const CharT* oldStr, const CharT* newStr
		) noexcept
		{
			using StrW = StringWriter<CharT>;
		...
		template <class StringT, typename = std::enable_if_t<
		          ...
		          >>
		USE_RESULT constexpr StringWriter<typename StringT::value_type> Replace(
			const StringT& str, const StringT& oldChar, const StringT& newChar
		) noexcept
		{
		...
		template <class StringT, typename = std::enable_if_t<
		          ...
		          >>
		USE_RESULT constexpr StringWriter<typename StringT::value_type> Replace(
			const StringT& str, typename StringT::value_type oldChar, typename StringT::value_type newChar
		) noexcept
		{
		...
		USE_RESULT constexpr std::vector<StringWriter<CharT>> Split(
			const StringWriter<CharT>& str, const CharT* tokens,
		...
		template <class CharT = char>
		USE_RESULT constexpr std::vector<StringWriter<CharT>> Split(
			const StringWriter<CharT>& str, const StringReader<CharT>& tokens,
			const PreserveToken preserveToken = PreserveToken::NO, PreserveEmpty preserveEmpty = PreserveEmpty::NO
		)
		{
